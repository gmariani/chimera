...
importScripts('./swf-lib/cff.js');
importScripts('./swf-lib/wav.js');
importScripts('./swf-lib/zip.js');
importScripts('./swf-lib/flv.js');
importScripts('./swf-lib/gif.js');
importScripts('./swf-lib/jpeg.js');
importScripts('./swf-lib/png.js');
importScripts('./swf-lib/pbj.js');
importScripts('./swf-lib/swf.js');
importScripts('./swf-lib/xml.js');
importScripts('./swf-lib/constants.js');
importScripts('./swf-lib/importTags.js');
importScripts('./swf-lib/importTypes.js');
importScripts('./swf-lib/tagCodes.js');
...
function readTags(ba, obj) {
	...
	while (header) {
		...
		var strTrace = 'LOG - ' + ba.position + ' - ' + header.name + ' (' + header.type + ') - ' + header.contentLength;
...
onmessage = function(event) {
	var id = event.data.fileID,
		file = new ByteArray( event.data.text, ByteArray.LITTLE_ENDIAN ),
		swf = {};
	
	swf.header = new SWFHeader( file );
	swf.streams = [];
	swf.dictionary = [];
	swf.stage = [new Frame()];
	...
	var ba = swf.header.byteArray;
	delete swf.header.byteArray;
	if ( ba ) swf.tags = readTags( ba, swf );
	
	postMessage( { fileID: id, data: swf } );
};
